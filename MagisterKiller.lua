--[[
    Magister Killer
    Loop:
      1. If no Magister Soul Reaper task, get one (Death / Grim gem)
      2. Load last preset at the Wars bank chest
      3. Enter the Reaper portal (or attuned Magister portal)
      4. Enter The First Gate, touch the Soul obelisk
      5. Kill The Magister
      6. Repeat 4–5 until Key to the Crossing runs out
      7. Teleport to War's Retreat and repeat

    Requirements:
      * War's Retreat Teleport on the ability bar
      * Last bank preset includes Keys to the Crossing + kill setup
      * Grim gem in inventory (or Death in range) for reaper tasks
      * Reaper portal at Wars, or a boss portal attuned to The Magister
      * 115 Slayer, revolution bar ready
]]
...
-- Arch-Glacor is child 42. Magister is child 28.
...
-- AG snippet: API.GetVarbitValue(22901) ~= 0. Only call once IsCacheLoaded().
...
-- Same as the Arch-Glacor snippet: grim gem "Assignment" (action 7 / route2).
...
-- Arch-Glacor uses child 42. Magister is child 28.
...
-- First enable before the arena. During the fight, re-enable if it drops.
...
-- Overload, elven shard, prayer potions, super restores — instance only.
...
local function groundItems(distance)
    ...
    -- GetAllObjArray1 is what Kerapac/Rasial use for floor piles. Do not
    -- require type(list)=="table": ReadAllObjectsArray can return userdata.
    pcall(function()
...
-- Rotate open methods. Returning true from Loot_w/Direct does not mean the
-- window opened, so never stick on one call.
local function clickGroundItemToOpenLoot()
    ...
    if method == 2 then
        -- RS3 default area-loot key. ClueUtils opens the loot table this way.
...
-- Obelisk / gate / reaper confirmation dialogs.
...
local function doGetTask()
    ...
    -- After Magister is selected: first dialog = space, second = 1.
...
local function doLoot()
    -- Stay in loot until the window is handled. A dead Magister NPC plus a
    -- sticky combat flag used to bounce this state back to FIGHT.
    if isMoving() then
        return
    end
    ...
    -- Keep trying to open for a while even if the scan is empty (drops lag).
